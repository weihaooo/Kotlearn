...
import com.assignment.kotlearn.kotlearn.R.id.*
...
class LoginActivity : AppCompatActivity(), LoaderCallbacks<Cursor> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //Auto Redirect if alr logged in
        if (sharedPreferences.getString(ISLOGGEDIN, EMPTY) != EMPTY) {
            val isLoggedIn = sharedPreferences.getString(ISLOGGEDIN, EMPTY)
            if (isLoggedIn == "true" && sharedPreferences.getString(USERNAME, EMPTY) != EMPTY) {
    ...
    inner class UserLoginTask internal constructor(private val mUsername: String, private val mPassword: String) : AsyncTask<Void, Void, Boolean>() {
        ...
        override fun doInBackground(vararg params: Void): Boolean? {
            ...
            try {
                ...
                //Thread.sleep(500)
                var result: Boolean = userDBHelper.login(mUsername,mPassword)

                //If successful, store isloggedin true and username
                if (result) {
        ...
        override fun onPostExecute(success: Boolean?) {
            ...
            Log.d(ContentValues.TAG, "SUCCESS IN POST " + success.toString())
            if (success!!) {
                val intent = Intent(this@LoginActivity, BottomNav1::class.java)
                startActivity(intent)
                //main
            } else {
                password.text = null
